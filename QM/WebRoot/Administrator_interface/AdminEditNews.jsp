	...
	<div class="main">
		...
		<div class="logo">
			...
			<div class="Login_box clearfix">
				...
				<a href="/QM/IndexList" class="Member_login">回到首页</a> <a
				...
				<c:if test="${sessionScope.admin2 != null}">
					<div class="welcome fr" id="welcome">
						欢迎回来：<span>${sessionScope.admin2.ad_name}</span> <a
							href="/QM/AdminInfoServlet?id=${admin2.id }" class="welcome_edit">编辑</a>
		...
		<div class="content_2">
			<form action="/QM/UpDateNewsInfo?id=${news.id }" method="post" enctype="multipart/form-data">
				<div class="newsBox">
					<p>请修改协会新闻信息</p>
					<input type="hidden" name="id" value="${news.id }">
					<p>
						新闻照片： <img src="/QM/ImgServlet?imgurl=${news.news_img}" style="cursor: pointer; width:100px;height:50px;"/>
						        <input type="file" value="" name="news_img" />
					</p>
					<p>
						新闻标题：<input type="text" value="${news.news_titles }" name="news_titles" />
					</p>
					<p>
						新闻时间：<input type="text" value="${news.news_time }" name="news_time" />
					</p>
					<p>
						新闻内容：<input type="text" value="${news.news_content }" name="news_content" style="height:60px;word-break:break-all;" />
					</p>
					<p>
						具体内容： <input type="text" value="${news.news_allcont }" name="news_allcont"  style="height:60px;"/>
					
					</p>
					<p>
						新闻链接：<input type="text" value="${news.news_deily_cont }" name="news_deily_cont"  />
					</p>
										
					<p class="addNews_box">
						<input type="submit" class="addNewsBtn" name="addNewsBtn" id="addNewsBtn" value="修   改" /> 
						<input type="reset" class="addNewsreset" name="reset" id="reset" value="重  置" />
					</p>
				</div>
			</form>

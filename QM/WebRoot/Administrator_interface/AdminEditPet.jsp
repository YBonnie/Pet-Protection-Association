...
<body>
	<div class="main">
		...
		<div class="content_2">
			<form action="/QM/UpDatePetInfo?id=${pet.id }" method="post" enctype="multipart/form-data">
				<div class="newsBox">
					<p>请修改用户信息</p>
					<input type="hidden" name="id" value="${pet.id }">
					<p>
						宠物属性：<input type="text" value="${pet.kind }" name="kind" />
					</p>
					<p>
						宠物名字：<input type="text" value="${pet.name }" name="name" />
					</p>
					<p>
						宠物年龄：<input type="text" value="${pet.age }" name="age" />
					</p>
					<p>
						宠物性别：<input type="text" value="${pet.sex }" name="sex" />
					</p>
					<p>
						宠物品种：<input type="text" value="${pet.type }" name="type" />
					</p>

					<p>
						领养状态：<input type="text" value="${pet.status }" name="status"  placeholder="0 / 1"/>
					</p>
					<p>
						宠物照片： <img src="/QM/ImgServlet?imgurl=${pet.imgurl}" style="cursor: pointer; width:100px;height:50px;"/>
						        <input type="file" value="" name="imgurl" />
					</p>
					<p class="addNews_box">

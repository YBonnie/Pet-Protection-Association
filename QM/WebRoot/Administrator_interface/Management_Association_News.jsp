...
<body>
		<div class="main">
			...
			<div class="content_1">
					...
					<form action="/QM/FindNewsTitle"  method="post">
						<label>协会新闻查询：</label>
						<input type="text" class="text_1" name="FindNewsTitle" id="FindNewstitle" value="" placeholder="请输入新闻标题" style="height:30px;" />
							<input type="submit" value="查 询" class="FindUserNameBtn">
							<a href="/QM/Administrator_interface/AdminAddNews.jsp">
								<span class="add_btn" id="add_btn" >添加协会新闻</span> 
							</a>
					</form>
					
					</div>
				<form action="/QM/BatchDelNewsServlet" method="POST">
					<table class="table_3" border="0">
						...
						<c:forEach items="${requestScope.NewsList}" var="news">
							<tr class="tbody">
							  <td><input type="checkbox" name="delId" value="${news.id }" style="width:65px;" /></td>
							 <td><img src="/QM/ImgServlet?imgurl=${news.news_img }" style="cursor: pointer;width:110px;height:80px;"/></td>
								<td class="newTd">${news.news_titles }</td>
								<td>${news.news_time }</td>
								<td class="newTd">${news.news_content }</td>
								<td class="newTd">${news.news_allcont }</td>
								<td><a href="/QM/newsContent?id=${news.id}" style="color:red">${news.news_deily_cont }</a></td>
								<td>
									<a href="/QM/DelNewsServlet?id=${news.id }"><span class="delect_btn" id="delect_btn">删除</span></a>
									<a href="/QM/EditNewsServlet?id=${news.id }"><span class="edit_btn" id="edit_btn">修改</span></a>
								</td>
							</tr>
						</c:forEach>
					</table>

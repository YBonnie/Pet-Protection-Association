		...
		<div class="main">
			...
			<div class="content_1">
					...
					<div class="check_table_1">
						...
						<form action="/QM/FindCommentTimer" method="post">
						<label for="">留言信息查询：</label>
						<input type="text" class="text_1" name="FindCommentTimer" id="FindCommentTimer" value="" placeholder="请输入留言时间" />
				...
				<form action="/QM/BatchDelMsgServlet" method="post">
				<table class="table_3" border="0">
						...
						<tr class="tbody">
						<td><input type="checkbox" name="delId" value="${comment.id }" style="width:65px;" /></td>
							<td>${comment.id }</td>
							<td>${comment.username }</td>
							<td>${comment.time }</td>
							<td>${comment.text }</td>
							<td>
							<a href="/QM/DelCommentsServlet?id=${comment.id }"><span class="delect_btn" id="delect_btn">删  除</span></a>
							</td>
						</tr>
					</c:forEach>

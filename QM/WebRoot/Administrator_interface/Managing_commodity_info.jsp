...
<body>
		<div class="main">
			...
			<div class="content_1">
					...
					<form action="/QM/FindGoodsName" method="post">
						<label for="">商品信息查询：</label>
						<input type="text" class="text_1" name="FindGoodsName" id="FindGoodsName" value="" placeholder="请输入商品的名字" />
						<input type="submit" value="查 询" class="FindUserNameBtn">
			
						<a href="/QM/Administrator_interface/AdminAddGoods.jsp">
							<span class="add_btn" id="add_btn" >添加商品信息</span> 
					    </a>
					</form>
					
					</div>
					<form action="/QM/BatchDelGoodsServlet" method="post">
					<table class="table_3" border="0">
						...
						<tr class="tbody">
						<td><input type="checkbox" name="delId" value="${goods.id }" style="width:65px;" /></td>
							<td>${goods.id }</td>
						<td><img src="/QM/ImgServlet?imgurl=${goods.imgurl}" style="cursor: pointer;"/></td> 
								<!--  <td><img src="${goods.imgurl}" alt="" /></td>-->
							<td>${goods.name }</td>
							<td>${goods.price }</td>
							<td>${goods.total_number }</td>
							<td>
							<a href="/QM/DelGoodsServlet?id=${goods.id }"><span class="delect_btn" id="delect_btn">删  除</span></a>
							<a href="/QM/EditGoodsServlet?id=${goods.id }"><span class="edit_btn" id="edit_btn">修  改</span></a>
							</td>
						</tr>
					</c:forEach>
					</table>

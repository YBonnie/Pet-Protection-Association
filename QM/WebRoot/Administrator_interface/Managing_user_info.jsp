		...
		<!--logo栏-->
			...
			<div class="content_1">
					...
					<div class="check_table_1">
						<form action="/QM/FindUserName"  method="post">
							<label for="">用户信息查询：</label>
							<input type="text" name="FindUserName" id="" value="" placeholder="请输入登录账户名" style="height:30px;"/>
							<input type="submit" class="FindUserNameBtn" value="查 询" >
						</form>
					</div>
				<form action="/QM/BatchDelUserServlet" method="POST">
					...
					<c:forEach items="${requestScope.UserList}" var="user">
						<tr class="tbody">
							  <td><input type="checkbox" name="delId" value="${user.id }" style="width:65px;" /></td>
							<td>${user.username }</td>
							<td>${user.password1 }</td>
							<td>${user.truename }</td>
							<td>${user.chineseId }</td>
							<td>${user.email }</td>
							<td>${user.phone }</td>
							<td>
								<a href="/QM/DelUserServlet?id=${user.id }"><span class="delect_btn" id="delect_btn">删除</span></a>
								<a href="/QM/EditUserServlet?id=${user.id }"><span class="edit_btn" id="edit_btn">修改</span></a>
							</td>
						</tr>
					</c:forEach>
					</table>

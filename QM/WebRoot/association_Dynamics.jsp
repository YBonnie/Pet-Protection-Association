		...
		<script>
			function changePage(obj){
  			if(isNaN(obj.value)){
  				...
  				window.location.href="/QM/DynamicsList?thispage="+obj.value;
  			}
  		}
  		</script>
</head>
<body>
			...
			<div class="breadcrumbs wow bounceInLeft" data-wow-duration="2s">
				...
				<a href="/QM/NewsList?thispage=1">&nbsp;&nbsp;新闻中心&nbsp;&nbsp;</a>
				<span> > </span>
				<a class="active" href="/QM/DynamicsList?thispage=1">&nbsp;&nbsp;协会动态&nbsp;&nbsp;</a>
			</div>

			<!--协会动态-->
			<div class="content wow fadeInUp" data-wow-duration="4s">
				<h1 class="wow fadeInUp" data-wow-duration="4s">协会动态</h1>
				<div class="content-l">
					<ul class="list">
							<c:forEach items="${requestScope.page.list1}"  var="dynamics">
							   <li>
								<span>${dynamics.dynamics_time}</span>
						        <span class="jiaoban"></span>
								<a href="/QM/dynaContentServlet?id=${dynamics.id}" title="${dynamics.dynamics_title}">${dynamics.dynamics_title}</a>
							   </li>
					       </c:forEach>
					</ul>
					...
					<div class="ep-pages">
					  	...
					  	<a href="/QM/DynamicsList?thispage=${page.firstpage}">首页</a>
					  	<a href="/QM/DynamicsList?thispage=${page.prepage}">上一页</a>
					  	...
					  	<c:forEach begin="${begin}" end="${end}" step="1" var="i">
					  		...
					  		<c:if test="${i != page.thispage}">
					  			<a href="/QM/DynamicsList?thispage=${i}">${i }</a>
					  		</c:if>
					  	</c:forEach>
					  	
					  	<!-- 分页逻辑结束 -->
					  	
					  	<a href="/QM/DynamicsList?thispage=${page.nextpage }">下一页</a>
					  	<a href="/QM/DynamicsList?thispage=${page.lastpage }">尾页</a>

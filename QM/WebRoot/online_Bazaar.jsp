...
<script>
			function changePage(obj){
  			if(isNaN(obj.value)){
  				...
  				window.location.href="/QM/BazaarList?thispage="+obj.value;
  			}
  		}
		</script>
</head>
<body>
	<div class="main">
		...
		<div class="breadcrumbs wow bounceInLeft" data-wow-duration="2s">
			<a href="/QM/IndexList">&nbsp;&nbsp;首页&nbsp;&nbsp;</a> <span> > </span> <a
				href="/QM/BazaarList?thispage=1" class="active">&nbsp;&nbsp;线上义卖&nbsp;&nbsp;</a>

		</div>

		<!--content-->
		<div class="content wow fadeInUp" data-wow-duration="4s">
			<h1 class="wow bounceInLeft" data-wow-duration="3s">线上义卖</h1>
			<ul class="pet_list clearfix wow fadeInUp" data-wow-duration="4s">
				<c:forEach items="${requestScope.page.list4}" var="good">
					<li>
						<p class="p1">
							<img src="/QM/ImgServlet?imgurl=${good.imgurl}">
						</p>
						<p class="p2">名字：${good.name }</p> <a
						href="/QM/GoodInfo?id=${good.id} " target="_blank">
							<p class="p3">详 情</p> </a></li>
			...
			<div class="ep-pages">
					  	...
					  	<a href="/QM/BazaarList?thispage=${page.firstpage}">首页</a>
					  	<a href="/QM/BazaarList?thispage=${page.prepage}">上一页</a>
					  	...
					  	<c:forEach begin="${begin}" end="${end}" step="1" var="i">
					  		...
					  		<c:if test="${i != page.thispage}">
					  			<a href="/QM/BazaarList?thispage=${i}">${i }</a>
					  		</c:if>
					  	</c:forEach>
					  	
					  	<!-- 分页逻辑结束 -->
					  	
					  	<a href="/QM/BazaarList?thispage=${page.nextpage }">下一页</a>
					  	<a href="/QM/BazaarList?thispage=${page.lastpage }">尾页</a>

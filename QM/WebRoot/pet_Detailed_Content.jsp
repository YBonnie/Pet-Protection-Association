...
<body>
			...
			<div class="breadcrumbs wow bounceInLeft" data-wow-duration="2s">
				...
				<a href="base_Introduction.jsp">&nbsp;&nbsp;宠物基地&nbsp;&nbsp;</a>
				<span> > </span>
				<a class="active" href="/QM/PetInfoList?thispage=1">&nbsp;&nbsp;宠物介绍&nbsp;&nbsp;</a>
			</div>

			<!--宠物介绍内容-->
			<div class="content wow fadeInUp" data-wow-duration="4s">
				<h1 class="wow bounceInLeft" data-wow-duration="3s">宠物详细信息</h1>
				<div class="pet_Information clearfix wow fadeInUp" data-wow-duration="4s">
					<img src="/QM/ImgServlet?imgurl=${requestScope.pet.imgurl}" alt=""  class="fl"/>
					<div class="pet_Info_box fl">
						<p>芯片：${requestScope.pet.id}</p>
						<p>属性：${requestScope.pet.kind}</p>
						<p>姓名：${requestScope.pet.name}</p>
						<p>年龄：${requestScope.pet.age}</p>
						<p>性别：${requestScope.pet.sex}</p>
						<p>品种：${requestScope.pet.type}</p>
						<p>领养情况：
							<c:if test="${requestScope.pet.status==0}">
							   <font color="blue">未被领养</font>
							</c:if>
							<c:if test="${requestScope.pet.status==1}">
								<font color="red">已被领养</font>
							</c:if>
						</p>
					</div>
				</div>
				<div class="lingyang wow fadeInUp" data-wow-duration="4s">
						<img src="media/images/lingyang_icon.gif" alt="" />
						<p>尊敬的朋友，你愿意加入动物领养者的行列，为流浪动物尽一份自己的力量吗？ 您可以在周末看望它，带它走动奔跑，给它带来欢乐和温暖。希望您能伸出援手，让被遗弃的动物不再受无家可归之苦！</p>
						<a href="adoption_Form.jsp"><span>我 要 领 养</span></a>
					</div>
			</div>
